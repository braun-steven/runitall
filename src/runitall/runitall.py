...
def run_shell_command_with_resource(
    ...
    logger.info(f"[{resource}] Running command: " + command)
...
if __name__ == "__main__":
    ...
    task2 = ShellTask(
        command=cmd_quick,
        depends_on=[task1] # Task 2 will wait for Task 1
    )
    ...
    if not gpu_resources:
        logger.error("No GPU resources parsed. Exiting. Check parse_resource_integer_string input.")
    elif not all_tasks:
        logger.error("No tasks defined. Exiting.")
    else:
        logger.info(f"Running {len(all_tasks)} tasks on {len(gpu_resources)} resources.")
        run_tasks(
            tasks=all_tasks,
